...
function renderMapPanel() {
  ...
  const cards = zones.map((z, idx) => {
    const unlocked = idx < state.unlockedZones;
    return `<div class="system-card"><div class="flex between"><strong>${z.name}</strong><span class="badge-tier t${z.allowedMaterialTiers[0]}">T${z.allowedMaterialTiers.join('/')}</span></div><div class="small muted">Lv ${z.recommendedLevel}</div><div class="small">Drops: Tier ${z.allowedMaterialTiers.join(',')}</div><div class="small ${unlocked ? '' : 'muted'}">${unlocked ? 'Unlocked' : 'Locked'}</div></div>`;
  }).join('');
